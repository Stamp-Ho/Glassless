...
- `query`에 더해 `region`과 `category`를 함께 보내면 더 정확하게 후보를 좁힐 수 있음
- `category`는 빈 문자열로 보내도 서버에서 `null`로 정규화됨
...
- `POST /api/comments/posts/{post_id}` — create a comment for a post
...
Notes:
- Comments carry `nickname`, `password`, and `content` and are linked to a parent `post_id`.
